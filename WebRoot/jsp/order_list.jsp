...
<body>
	...
	<div class="container">
		<div class="row">
			...
			<div style="margin:0 auto; margin-top:10px;width:950px;">
				...
				<table class="table table-bordered">
					<c:forEach items="${bean.list }" var="order">
						<tbody>
							<tr class="success">
								<th colspan="5">订单编号:${order.oid } 订单金额：${order.total } <c:if
										test="${order.state==0 }">
							...
							<c:forEach items="${order.items }" var="oi">
								...
								<tr class="active">
									...
									<td width="30%"><a target="_blank">${oi.product.pname
											}</a></td>
		...
		<div style="text-align: center;">
			<ul class="pagination">
				<c:if test="${1==bean.currPage }">
					<li class="disabled"><a href="javascript:void(0)"
						aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
				</c:if>
				<c:if test="${1!=bean.currPage }">
					<li><a
						href="${pageContext.request.contextPath}/order?method=findAllByPage&currPage=${bean.currPage-1}"
						aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
				</c:if>
				...
				<c:forEach begin="1" end="${bean.totalPage }" var="n">
					...
					<c:if test="${n!=bean.currPage }">
						<li><a
							href="${pageContext.request.contextPath}/order?method=findAllByPage&currPage=${n}">${n
								}</a></li>
					</c:if>
				</c:forEach>

				<c:if test="${bean.totalPage==bean.currPage }">
					<li class="disabled"><a href="javascript:void(0)"
